...
\begin{itemize}[<+->]
    \item Objectives and scope using cognitive theories
    \item French narratives based on emotion components
    \item Models for emotion analysis in emotional and dream narratives 
...
\Large
\begin{center}
    Objectives and scope using cognitive theories
    %\section{Objectives and scope using cognitive theories}
\end{center}
\vspace{1.5cm}
...
\begin{itemize}[<+->]
    ...
    \item Though the three theories reviewed are usually considered rivals, their integration is possible and desirable \smallcitep{schererTheoryConvergenceEmotion2022}
...
$\rightarrow$ Different emotion expression modes are more or less difficult to interpret \smallcitep{davidsonChildrenRecallEmotional2001,blancComprehensionContesEntre2010,creissenQuelleRepresentationDifferentes2017a,ball_conversational_2024}
...
$\rightarrow$ [add refs] \smallcitep{etienne-etal-2022-psycho,dragos-etal-2022-angry}
...
$\rightarrow$ We use this framework to construct an emotion dataset 
...
\textbf{Limitation}: No annotation schemes consider all emotion components

\vspace{0.5cm}
\pause

\citet{kim-klinger-2019-analysis} study emotion communication in fiction via subjective sensations, postures, and facial expressions

\vspace{0.5cm}
\pause

\citet{casel-etal-2021-emotion} associate text spans with Scherer's emotion components

%\vspace{0.25cm}
%\pause

%To our knowledge, no annotation schemes capture the interaction between components

\vspace{0.5cm}
\pause

$\rightarrow$ We structure emotional narratives according to behaviors, thoughts, physical feelings, and appraisals
...
\onslide<5->{
$\rightarrow$ Some components benefit from contextual understanding and world knowledge (\texttt{behavior} and \texttt{thinking})
}
...
\begin{frame}{Motivation for dream analysis}
  ...
  We performed emotional analysis on concrete, real life situations
...
%\begin{itemize}
%    \item Status: individual alive (1), group alive (2), dead individual (3), dead group (4), imaginary individual (5), imaginary group (6), original form (7), changed form (8).
%    \item Gender: male (M), female (F), joint (J), indefinite (I).
%    \item Identity: known (K), prominent (P), occupational (O), ethnic (E), stranger (S).
%    \item Age: adult (A), child (C).
%\end{itemize}
...
\begin{frame}{Existing research on computational dream analysis}
  ...
  This approach uses full narrative context and compares predictions with HVdC gold annotations

  \vspace{0.5cm}
  \pause

  \textbf{Limitations}: emotions are not linked to characters; emotion frequency is not modeled

  \vspace{0.5cm}
  \pause
  
  $\rightarrow$ We address this by identifying characters and their emotions with transformer-based sequence-to-sequence models
...
\begin{table}
  ...
  \begin{tabular}{lcccccc}
    ...
    \texttt{No\textsubscript{names}} & \textcolor{bluepanam}{80.7}  & \textcolor{blueNCS}{74.3}  & 74.2  & \textcolor{bluepanam}{84.0}  & \textcolor{blueNCS}{60.9} &  \textcolor{bluepanam}{73.0} \\
    \hline
    
    \pause
    
    \texttt{Size\textsubscript{small}} & \textcolor{blueNCS}{78.4}  & \textcolor{blueNCS}{72.1}  & \textcolor{blueNCS}{70.3}  & \textcolor{blueNCS}{81.7}  & \textcolor{blueNCS}{56.8} &  \textcolor{blueNCS}{70.2} \\
    \texttt{Size\textsubscript{large}} & \textcolor{bluepanam}{84.5} & \textcolor{blueNCS}{80.3}  & \textcolor{blueNCS}{78.6}  & 87.3  & \textcolor{blueNCS}{67.6} & 74.7 \\
    \hline
    
    \pause
    
    \texttt{First\textsubscript{group}} & 82.3  & 77.7  & 74.9  & 85.6  & 63.7 &  71.9 \\
    \texttt{First\textsubscript{individual}} & \textcolor{blueNCS}{80.6}  & 76.1  & \textcolor{bluepanam}{74.2}  & \textcolor{blueNCS}{83.9} & 62.7 &  67.3 \\
    \texttt{First\textsubscript{emotion}} & 83.9 & 78.7 & 77.1  & 87.6  & 65.0 &  72.0 \\
    \hline
    
    \pause
    
    \texttt{Conversion\textsubscript{comma}} & \textcolor{blueNCS}{84.0}  & \textcolor{blueNCS}{79.8}  & \textcolor{blueNCS}{77.7}  & \textcolor{bluepanam}{87.1}  & \textcolor{blueNCS}{66.7} &  73.7 \\
    \texttt{Conversion\textsubscript{marker}} & 82.4  & 78.5  & 76.5  & 86.1  & 65.4 &  74.4 \\
  \end{tabular}
  
  \caption{\textbf{Character and emotion detection}. \scriptsize{\textcolor{bluepanam}{$p<0.05$}, \textcolor{blueNCS}{$p<0.01$}.}}
...
\begin{frame}{Introduction}
  ...
  \vspace{0.5cm}
    ...
    In NLP, style transfer aims to control linguistic attributes while preserving semantic content \smallcitep{jinDeepLearningText2022}
...
\begin{frame}{Introduction}
  ...
  $\rightarrow$ We aim to create a accessible framework that researchers can build upon in future studies % We do not claim to capture the full complexity of subjective experience in text

\end{frame}

\begin{frame}{How to give an operational definition of style?}

\textbf{Intuitive definition}: a distinctive manner of communicating subjective experience in narratives
...
\begin{itemize}[<+->]
    \item \textbf{Authorship profiling}: predicting authors based on their sequences \smallcitep{ferraraDetectionPromotedSocial2016}
    ...
    \item \textbf{Style-conditioned narrative generation} \smallcitep{alhafniPersonalizedTextGeneration2024b} [add pachet]%: generating narratives from a sequence of linguistic features
...
\section{Thematic analysis in mental health narratives}
...
\begin{frame}{Perspectives}
  ...
  \begin{itemize}[<+->]
    \item \textbf{Emotion analysis for mental health}: empathic support \smallcitep{sharmaCognitiveReframingNegative2023b}, cognitive distortions, theory of mind \smallcitep{zhouHowFaRAre2023,maHolisticLandscapeSituated2023,kimFANToMBenchmarkStresstesting2023,gandhiUnderstandingSocialReasoning2023}
    \item \textbf{Psychology of language models}: sycophancy, thought operations \smallcitep{didolkar2025metacognitivereuseturningrecurring} [add refs]
    \item \textbf{Post-training for psychology} (preferences and reasoning data) [add refs]
  \end{itemize}
