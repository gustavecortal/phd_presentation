...
\begin{itemize}\setlength{\itemsep}{0.2em}
  ...
  \item \textbf{Identity}: known (\textbf{K}), prominent (\textbf{P}), occupational (\textbf{O}), ethnic (\textbf{E}), stranger (\textbf{S})
  \item \textbf{Age}: adult (\textbf{A}), child (\textbf{C})
\end{itemize}
...
\begin{frame}{T5 sequence-to-sequence framework}
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{img/t5_architecture.png}
    %\caption{\textbf{T5 architecture} \smallcitep{2020t5}. The model is pre-trained on a text-to-text objective and can be fine-tuned for various NLP tasks.}
    \label{fig:placeholder}
\end{figure}
...
248M-parameter LaMini-Flan-T5 encoder-decoder transformer, pre-trained on 2.58M instructions across 15 tasks \smallcitep{lamini-lm}%, which takes token sequences as input and generates token sequences as output, and was among the top sub-1B models across 15 NLP tasks \parencite{lamini-lm}.
  ...
  \begin{column}{0.7\textwidth}
    ...
    \onslide<3->{
        ...
        \begin{itemize}
            ...
            \item \textit{Ideational}: language represents experience
        \end{itemize}
    }
  \end{column}
...
\textbf{Alphabet}: Let $\Sigma$ be a finite alphabet grounded in the ideational function of language
...
$$
\phi(T)=u\in\Sigma^n,\quad u=(u_1,u_2,\ldots,u_n)
$$

\vspace{0.25cm}
\pause

$\rightarrow$ We implement $\phi$ using a language model with in-context learning% to extract features at the clause level
...
Narratives and clinical scores from \textit{four cohorts}: French general population (n=1809) and three clinical cohorts (Italian n=116, Chinese n=52, Spanish n=90)

\vspace{0.5cm}
\pause

\textit{Clinical scores} for depression, anxiety, insomnia

\vspace{0.5cm}
\pause

\textit{Open-ended questions} involving last 24h, positive future event, current feelings and sleep, etc.
