...
\begin{frame}{Motivation}
  ...
  \citet{pachet} proposes constrained Markov processes to generate song lyrics that match an author's style while satisfying rhyme and meter constraints %style as markov chains, short-term probabilities, music domain

  \vspace{0.5cm}
  \pause
  
  $\rightarrow$ Drawing on these ideas, we provide a sequence-based framework to analyze how personal narratives convey subjective experience
...
\textbf{Alphabet}: Let $\Sigma$ be a finite alphabet grounded in \textit{Systemic Functional Linguistics} to capture meaningful distinctions in experience

\vspace{0.25cm}
\pause

$$ \Sigma = \{ \texttt{Action}, \texttt{Mental}, \texttt{Verbal}, \texttt{State}, \dots \} $$

\vspace{0.5cm}
\pause

\textbf{Mapping}: Let $\phi$ be a function that maps a narrative text $T$ to a sequence of symbols in $\Sigma^*$

\vspace{0.25cm}
\pause

$$ \phi(T) = u = (u_1, u_2, \dots, u_n) $$

\vspace{0.25cm}
\pause

$\rightarrow$ We implement $\phi$ using language models with in-context learning to extract features at the clause level

\end{frame}

\begin{frame}{Formal definition of style}

\textbf{Definition}: We define the style of text $T$ as the set of patterns contained in its sequence $\phi(T)$
...
\centering
\small
(where $w \subseteq u$ denotes that $w$ is a substring of $u$)
...
$\rightarrow$ Style is formally captured as the set of sequential patterns chosen by the author to encode experience
...
\begin{itemize}[<+->]
    \item \textbf{Incorporating additional linguistic features} with a functional role (duration of processes, concreteness of participants) %\smallcitep{hallidayIntroductionFunctionalGrammar2014a,troianoTheoriesStylesTheir2023a}%: adding features related to the interpersonal and textual functions of language
