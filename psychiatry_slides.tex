...
\addbibresource{gustave.bib}
\addbibresource{rachid.bib}
...
\institute{\footnotesize gustavecortal@gmail.com}
\titlegraphic{%
...
\begin{frame}{Context}
    ...
    \begin{tikzpicture}[scale=0.54, every node/.style={scale=0.58}]
        ...
        \node [circle, fill=white, inner sep=2pt, font=\bfseries\large, align=center] at (0,0) {My\\research};
...
\begin{itemize}[<+->]
    ...
    \item Emotion analysis in real-life and oniric situations
...
\vfill
\begin{center}
    Language models for emotion analysis\\ in real-life and oniric situations
...
\begin{figure}
  \centering
  \includegraphics[width=0.2\linewidth]{img/logo_callyope.png}
  \caption*{\href{https://www.callyope.com/}{callyope.com}}
  \label{fig:placeholder}
\end{figure}
...
\begin{frame}{Motivation}
  ...
  \begin{itemize}[<+->]
    ...
    \item Thematic analysis is time-consuming, often constrained to small, monolingual corpora \smallcitep{stanghellini2023lived}
...
\begin{frame}{Methodology}
  ...
  \begin{itemize}[<+->]
    \item clusters narratives across different cohorts by semantic content
    \item generates fine-grained natural language descriptions for each cluster
    \item links clusters to variation in clinical scores and sociodemographics
  \end{itemize}
...
\textbf{Clinical scores} for depression (BDI, PHQ9, MADRS), anxiety (GAD7), insomnia (AIS), and fatigue (MFI)
\vspace{0.5cm}
\pause

\textbf{Open-ended questions} involving last 24h, negative past event, positive future event, current feelings and sleep, etc.
...
\begin{frame}{Contributions}
  ...
  \begin{itemize}[<+->]
    \item First topic modeling \textit{across different languages and cohorts} using language models
    \item Replaced keywords \parensmall{("family")} with \textit{context-rich descriptions} to capture symptom co-occurrence and clinical nuance \parensmall{("family conflicts", "stress related to exam preparation")}
    \item Identified \textit{risk} \parensmall{("sleep disturbance")} and \textit{protective} \parensmall{("physical activity")} topics for depression, consistent with psychiatric literature
  \end{itemize}

  %language models can generate cluster descriptions reflecting core psychiatric constructs, bridging qualitative insights and scalable assessment
\end{frame}

\begin{frame}{Distribution of depression scores across clusters}

  \small{Current feelings and sleep (n=1786)}
  \begin{figure}
    \centering
...
\begin{frame}{Risk and protective topics for depression}

NLP and psychiatry researchers reviewed generated cluster descriptions to identify risk and protective topics for depression
...
\begin{itemize}[<+->]
  \item \textbf{Risk topics}: sleep disturbance and fatigue \smallcitep{yasugaki_bidirectional_2025}; unemployment and financial stress \smallcitep{arena_mental_2023}; exam pressure \smallcitep{perez-jorge_examining_2025}.
  \item \textbf{Protective topics}: arts and creative activity \smallcitep{fancourt_what_2019}; gardening and nature \smallcitep{soga_gardening_2017}; holidays and travel \smallcitep{de_bloom_vacation_2013}; physical activity \smallcitep{pearce_association_2022}
\end{itemize}

\vspace{0.5cm}
\pause

$\rightarrow$ We found topics consistent with psychiatric literature, aligned with labor-intensive qualitative research \smallcitep{stanghellini2023lived}
...
\vfill
\begin{center}
    Discussion% (change title to thematic analysis?)
    %\section{Thematic analysis in mental health narratives using language models}
\end{center}

\end{frame}

\begin{frame}{Comparison with keyword-based topic modeling}

Prior work on topic modeling in depression has relied on keyword-based approaches that surface topics as word lists. For instance, \cite{zhang_identifying_2024} applied BERTopic to free-response speech in the RADAR-MDD dataset and identified 29 topics represented by keywords such as ``nothing to look forward to,'' ``daughter, son, parents,'' ``hospital, operation, pain,'' and ``sleep, tired, rest.'' While such keywords indicate broad thematic areas, they lack the semantic context and narrative coherence that clinicians use when interpreting patient discourse.

Our approach produces qualitatively different outputs. Rather than keywords, we generate descriptions such as \textit{``sleep disturbances characterized by insomnia, frequent awakenings, and restless sleep, alongside pervasive anxiety, emotional instability, and self-esteem issues''} or \textit{``stress related to academic exams and significant life decisions.''} These descriptions capture the co-occurrence of symptoms, their subjective experience, and contextual factors in ways that keyword lists cannot. Moreover, our method requires no manual post-hoc categorization: the language model directly produces interpretable summaries, whereas keyword-based approaches typically require researchers to manually group and label topics based on their semantic meaning.

This distinction matters clinically. A keyword like ``sleep'' could appear in contexts ranging from satisfying rest to severe insomnia; our fine-grained descriptions disambiguate such cases. Similarly, ``family'' as a keyword does not distinguish between supportive family relationships (protective) and family conflict (risk), distinctions our method can capture through contextual summarization.
  
\end{frame}

\begin{frame}{Bridging qualitative research and clinical practice}

Our introduction highlighted that manual thematic analysis, while clinically valuable, is too resource-intensive for routine care \cite{stanghellini2023lived}. The present study demonstrates that language models can generate cluster descriptions that recapitulate core psychiatric constructs identified in labor-intensive qualitative research. For instance, our clusters containing sleep disturbance, emotional dysregulation, and impaired functioning align with the phenomenological dimensions of depression (altered temporal experience, embodied distress, and existential emptiness) identified through expert consensus \cite{stanghellini2023lived}.

This convergence suggests that automated methods may partially bridge the gap between what qualitative analysis reveals as clinically important and what clinicians can feasibly assess at scale. Our approach successfully identified clinically relevant content from speech records across diverse cohorts, even though some data were not originally collected for qualitative analysis. The language model generated meaningful syntheses using clinically agnostic prompts, without pre-established theory or domain-specific information.

However, automated methods complement rather than replace clinical judgment. The psychiatrist interpretation in Section \ref{subsec:qualitative_analysis} demonstrates that expert synthesis remains essential for contextualizing algorithmically discovered patterns and distinguishing clinically actionable insights from statistical artifacts.
  
\end{frame}

\end{comment}

%\begin{frame}{Potential clinical applications}

%While this study establishes methodological foundations, several clinical applications merit consideration. First, cluster-based analysis could support \textit{screening and risk stratification}: individuals whose speech patterns place them in high-severity clusters might be flagged for priority clinical review. Second, the fine-grained topic descriptions could inform \textit{personalized treatment planning}: a patient whose cluster emphasizes academic stress and sleep disturbance may benefit from different interventions than one whose cluster highlights social isolation and hopelessness. Third, tracking cluster membership or distance-to-cluster-centroid over time could enable \textit{treatment monitoring}, providing objective measures of thematic shifts during therapy.

%Critically, such applications require interpretability. Clinicians need to understand \textit{why} a patient was assigned to a particular cluster to trust and act on algorithmic recommendations. Our approach addresses this need by generating human-readable descriptions rather than opaque embeddings or keyword lists. This interpretability is essential for developing safe and trustworthy clinical decision support systems based on speech or language analysis. Future work should evaluate whether cluster descriptions improve clinician decision-making in controlled settings.
...
\begin{center}
  \textbf{contact}: gustavecortal@gmail.com
\end{center}
...
\begin{frame}{Potential applications and future work}

  \begin{itemize}[<+->]
    \item \textbf{Risk stratification}: flagging individuals in high-severity clusters for clinical review
    \item \textbf{Personalized treatment}: interventions based on cluster themes
    \item \textbf{Treatment monitoring}: tracking thematic shifts over time% during therapy
  \end{itemize}

  \vspace{0.5cm}
  %\pause

  $\rightarrow$ Do cluster descriptions improve clinician decision-making?
